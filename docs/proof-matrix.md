...
| --- | --- | --- | --- |
| `authorize-service` | `/oauth2/authorize` | Service-scope authorize request fields and immediate 302 with code/state. | Implemented; public stub run required. |
| `authorize-credential` | `/oauth2/authorize` | Credential-scope request includes credential ID, one signature, and SHA-256 consent hash; immediate 302. | Implemented; public stub run required. |
| `token-service` | `/oauth2/token` | Basic auth plus grant type, code, client ID, redirect URI; Bearer token response shape. | Implemented; public stub run required. |
| `credentials-list` | `/csc/v1/credentials/list` | Bearer JSON request and non-empty `credentialIDs` response. | Implemented; public stub run required. |
| `credentials-info` | `/csc/v1/credentials/info` | Selected ID, chain/certInfo request; SCAL 2, RSA-2048, auth mode, multisign and leaf identity response. | Implemented; public stub run required. |
| `token-credential-sad` | `/oauth2/token` | Credential authorization-code exchange has the same required form and returns SAD. | Implemented; public stub run required. |
| `sign-hash` | `/csc/v1/signatures/signHash` | Bearer request has ID, SAD, SHA-256 hash, `rsaEncryption` signAlgo, and one response signature. | Implemented; public stub run required. |
| `token-wrong-client` | `/oauth2/token` | Wrong HTTP Basic client credential is rejected (4xx). | Implemented; public stub run required. |
| `info-missing-credential` | `/csc/v1/credentials/info` | Missing credential ID is rejected (400). | Implemented; public stub run required. |
| `sign-hash-wrong-algorithm` | `/csc/v1/signatures/signHash` | `sha256WithRSAEncryption` is rejected (400); documented CSC `rsaEncryption` is the accepted value. | Implemented; public stub run required. |
| `sign-hash-invalid-sad` | `/csc/v1/signatures/signHash` | Invalid or expired SAD is rejected (400). | Implemented; public stub run required. |
