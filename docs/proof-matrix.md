...
default-pipeline evidence is the [`Tests` run on develop at aa6017c](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219).

| Claim | Mechanism that proves it | Independent validator | Latest green run | Not proven / not implemented |
| --- | --- | --- | --- | --- |
| Sign PAdES B-B with RSA | `TestCredentialFreeBB/v1_rsa` drives start, both OAuth legs, CSC `signHash`, CMS assembly and PDF embedding against the synthetic credential-free mock. | OpenSSL `cms -verify` over the PDF ByteRange and synthetic CA. | [Tests](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219) | Only the mock until a real acceptance/production run is recorded. |
| Sign PAdES B-T with RSA | `TestCredentialFreeBT/v1_rsa` performs the B-B journey plus the mock RFC 3161 TSA path. | OpenSSL CMS verification; timestamp token structure assertion. | [Tests](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219) | No qualified production TSA or acceptance signer run. |
...
| Sign with ECDSA P-256 | `TestCredentialFreeBB/v2_ecdsa` and `TestCredentialFreeECDSATamperRejected` drive CSC v2 raw `r∥s` normalization and reject a tampered result. | OpenSSL CMS verification. | [Tests](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219) | No Cleverbase-stub or acceptance ECDSA/v2 contract run. |
| Verify a singly-signed PAdES PDF (integrity only) | `verify_pdf` / Go `VerifyPDF` strictly parse the PDF signature dictionary and complete-`/Contents` ByteRange gap; require the PAdES signed-attribute profile; resolve the embedded signer by issuer+serial; verify RSA/P-256 ECDSA signed attributes; compare the CMS message-digest to the two signed PDF ranges; verify B-T token binding, embedded-token-signer signature and content digest; and report profile plus canonical signer serial/CN. `produced_b_b_signature_verifies_with_openssl`, `produced_b_t_signature_has_timestamp_and_verifies`, the copied/tampered timestamp-token tests, the PDF/CMS tamper tests, and `verifier_rejects_the_public_cleverbase_stub_fake_signature` cover the positive and negative vectors. | OpenSSL independently verifies the valid synthetic-PKI outputs. | [Tests](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219) | The SDK verifier does **not** build or trust signer/TSA chains, check revocation/trusted-list status or TSA policy. Multiple signatures are rejected. |
| Gateway sign → verify → tamper integration | trust-gateway v0.2.0 `TestBlackboxSigning` in mock mode completes both public-callback traversals, signs B-B/RSA, validates the CMS and PDF ByteRange, calls private `POST /v1/verify` on the resulting PDF, then changes a signed byte and requires `message_digest_mismatch`. The same job runs against Cleverbase's public stub and requires the fake signature to end as `SignatureInvalid`. | OpenSSL for the mock-produced PDF; Cleverbase's public hash-signing stub for the upstream contract path. | [trust-gateway black-box](https://github.com/alkem-io/trust-gateway/actions/runs/33998870023) | No trust-chain, revocation, signer-authorization, or qualified-status claim. The stub does not produce a signed PDF. |
| CA-backed validation of SDK-produced CMS | The reference-integration `verifyCMSWithCA` helper independently extracts the detached CMS and PDF ByteRange, then validates it against the supplied synthetic CA. | OpenSSL `cms -verify`. | [Tests](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219) | This is harness evidence for the synthetic mock chain, not certificate-path building by `verify_pdf` and not a Cleverbase acceptance/production trust result. |
...
| EUDI attestation verify | `conformance::iso_18013_5_annex_d_issuer_auth_verifies_under_the_sdk_verifier` verifies the vendored externally authored ISO/IEC 18013-5 Annex-D mdoc vector. | ISO/IEC 18013-5 Annex-D vector. | [Tests](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219) | This row does not prove a live Cleverbase issuer integration. |
| EUDI attestation issue / present / verify | Native issuance and wire tests under `issuance::{signer,present,wire}::tests` exercise SDK-issued material and its verifier. | None; these are SDK-controlled vectors. | [Tests](https://github.com/alkem-io/cleverbase-sdk/actions/runs/33995939219) | No independent live issuer proof in this row. |
| PAdES profile conformance | `.github/workflows/profile-conformance.yml` produces B-B/B-T RSA and P-256 PDFs and runs only when manually dispatched or a PR has the `profile-conformance` label. | Poppler `pdfsig` for PDF-native full-document signature validation, pyHanko for AdES validation, and EU DSS for the baseline-level assertion. | [Profile conformance](https://github.com/alkem-io/cleverbase-sdk/actions/runs/34465808166) | No signer/TSA qualified-status claim; those are separate trust decisions. |
| Production PAdES QES acceptance | A pilot user completed the Alkemio memo-signing flow against Cleverbase production and the Uanataca qualified TSA. The resulting single signature covers the full PDF with ByteRange `[0, 113375, 146145, 787]`. | EU DSS reported `QESig`, `PAdES-BASELINE-T`, `TOTAL_PASSED`, 1/1 valid signature, and a passed Uanataca TSU01 timestamp at 2026-09-10 15:14:04Z. | [Pinned HQ acceptance-evidence ledger](https://github.com/alkem-io/agents-hq/blob/140ad53b550fefb14814de1021d204b1cc37c8a5/specs/031-cleverbase-memo-signing/checkpoint-evidence.md#2026-09-10--acc-pilot-user-supplied-eu-dss-acceptance-evidence) (user-supplied record, 2026-09-10) | This proves the production artefact and service chain observed in that run. It does not turn the SDK's integrity-only verifier into a chain, revocation, trusted-list, or qualification validator. |
