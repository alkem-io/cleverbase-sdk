  ...
  if [ -z "$rust_channel" ]; then
    ...
    if [ -z "$rust_sysroot" ] || [ ! -x "$rust_sysroot/bin/cargo-clippy" ]; then
      ...
      if [[ "$clippy_version" != "$clippy_expected"* ]]; then
        ...
        run clippy-python env PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 rustup run "$rust_channel" cargo clippy --locked --manifest-path bindings/python/Cargo.toml --all-targets -- -D warnings
        run clippy-node rustup run "$rust_channel" cargo clippy --locked --manifest-path bindings/node/Cargo.toml --all-targets -- -D warnings
      fi
    fi
  fi
fi
